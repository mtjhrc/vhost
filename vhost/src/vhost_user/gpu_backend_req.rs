...
mod tests {
    ...
    fn test_gpu_backend_cannot_send_when_failed() {
        ...
        backend
            .send_message_with_payload_no_reply::<VhostUserEmpty>(
                GpuBackendReq::GET_PROTOCOL_FEATURES,
                &VhostUserEmpty,
                &[],
                None,
    ...
    #[test]
    fn test_gpu_backend_send_message_with_big_payload() {
        let (mut frontend, backend) = frontend_backend_pair();
        ...
        let sender_thread = thread::spawn(move || {
            backend
                .send_message_with_payload_no_reply(REQUEST, &VALID_REQUEST, &sending_data, None)
                .unwrap();
        });
